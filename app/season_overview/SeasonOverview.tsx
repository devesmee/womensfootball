...
import { API_BASE_URL, API_KEY_HEADER, LEAGUES_PATH, STANDINGS_PATH } from '../constants/Constants';
...
export default function SeasonOverview({ route, navigation }: Props) {
    ...
    const fetchStandings = async () => {
        ...
        try {
            const response = await axios.get<ApiResponse<Standing>>(API_BASE_URL + STANDINGS_PATH,
                {
                    headers: {
            ...
            if (response.data.response.length !== 0) {
                ...
                console.log('Something went wrong when fetching the standings: ', response.data.errors);
                setHasError(true);
            }
            setIsLoading(false);
        } catch (error) {
